...
(define (change-status-a a b n)
  (if (= a 1)
      (* b b)
      (* a a)))

(define (change-status-x a b x)
  (if (= a 1)
      (* b x)
      (* a x)))
...
(define (iter-expt n b a x)
  (cond ((= n 1) (if (= a 1)
                     b 
                     (* a x)))
        ((even? n) (iter-expt (/ n 2) b (change-status-a a b n) x))
        (else (iter-expt (- n 1) b a (change-status-x a b x)))))
...
(define (iter-expt n b a x)
  (cond ((= n 1) (* a x))
        ((even? n) (iter-expt (/ n 2) b (* a a) x))
        (else (iter-expt (- n 1) b a (* a x)))))
